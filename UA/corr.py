...
class CorrBlock1D:
    def __init__(self, fmap1, fmap2, num_levels=4, radius=4):
        ...
        corr = corr.reshape(batch*h1*w1, 1, 1, w2) ### 在视差方向上进行池化
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            x0 = dx + coords.reshape(batch*h1*w1, 1, 1, 1) / 2**i  ###每个像素点在x方向的位移
            y0 = torch.zeros_like(x0)  ###视差估计中，y方向位移为0
...
class CorrBlock1D_pro:
    def __init__(self, fmap1, fmap2, freq_volume, num_levels=4, radius=4):
        self.num_levels = num_levels
        self.radius = radius
        self.corr_pyramid = []

        self.freq_volume_pyramid = freq_volume  ### hw维度不同

        # all pairs correlation
        corr = CorrBlock1D_pro.corr(fmap1, fmap2)
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            ### freq_corr
            freq_corr = self.freq_volume_pyramid[i]
            freq = bilinear_sampler(freq_corr, coords_lvl)
            freq = freq.view(batch, h1, w1, -1)
            out_pyramid.append(freq)
